...
export function ProjectInputs({ projectId }: { projectId: string }) {
  ...
  const uploadAppearance = {
    ...
    label: { color: "var(--text-secondary)", fontSize: "0.8125rem", fontWeight: 500 },
  ...
  return (
    <div className="space-y-8">
      ...
      {error && (
        <div className="rounded-[var(--radius-sm)] bg-danger-soft px-3 py-2 text-xs font-medium text-danger">
          {error}
        </div>
      )}
        ...
        <div className="space-y-3">
          ...
          <label className="flex items-start gap-2.5 rounded-[var(--radius-md)] border border-border bg-surface-raised p-3">
            <input
              ...
              className="mt-0.5 rounded border-border text-text accent-[var(--accent)]"
            />
            <span className="text-[11px] leading-tight text-text-secondary">
              I confirm I have the right to use this likeness and will not generate public figures without consent.
            </span>
          </label>
              ...
              run(
                () =>
                  request(`/api/projects/${projectId}/avatar`, {
                    ...
                    policyState: avatarForm.sourceType === "ai_person" ? "ai_generated" : avatarForm.policyState,
