      ...
      <div className="max-h-[440px] space-y-3 overflow-y-auto px-5 py-4">
        ...
        {messages.map((message) => (
          ...
          >
            <div
              className={cn(
                "max-w-[88%] whitespace-pre-wrap rounded-[var(--radius-md)] px-3.5 py-2.5 text-[13px] leading-relaxed",
                message.role === "user"
                  ? "bg-text text-bg"
        ...
        <div className="mb-3 flex flex-wrap gap-2">
          {suggestions.map((suggestion) => (
            <button
              ...
              className="rounded-full border border-border bg-surface-raised px-2.5 py-1 text-[11px] font-medium text-text-secondary transition-colors hover:border-border-strong hover:text-text disabled:opacity-50"
            >
              {suggestion}
            </button>
          ))}
